...
- **Impact marks** — Quake 3-style decals: bullet holes, blaster burns,
  explosion scorches and the rail's blue energy splat stamped onto the
  walls where shots land, wrapping corners and fading out after a while.
  Fresh blaster/hyperblaster burns glow hot and shimmer with heat haze as
  they cool. Hits on flesh fling blood splatter onto nearby surfaces,
  every blood droplet that lands on a wall or floor sticks as a lasting
  splat, and a pool of blood — one of several irregular, runner-fingered
  shapes — spreads under monsters where they die. Blood never washes off:
  weapon marks fade after `gl_decaltime` seconds, but splats, pools and
  boot prints stay for the rest of the level. Walk through a death pool
  and your boots track fading bloody footprints for the next several
  steps (`gl_decals`, default on; textures generated by
  `tools/gen_decals.py` into `baseq2/textures/decals`)
- **Corpse flies** — the classic `EF_FLIES` swarm draws actual winged,
  wing-beating flies circling the body (Unreal-style) instead of id's
  black particle dots, lit by the world around them; the swarm gathers
  over time and stays for as long as the corpse lies there instead of
  id's 60-second shutoff (sprite generated by `tools/gen_fly_sprite.py`
  into `baseq2/sprites`)
